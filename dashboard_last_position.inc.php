...
$mapping = 'tst=Data/Ora#tracker=Tracker#lat=Lat.#lon=Lon.#t=Ev.#id=Id#view_all=Tracciamento#last1=Tracciamento#last2=Tracciamento#last4=Tracciamento#last8=Tracciamento#gmaps=Mappa Google';
$title = 'Ultimo tracciamento per dispositivo';

$stmt = 'SELECT DISTINCT g.tracker, ${tracker.risorsa} as Risorsa,g.tracker AS last1,g.tracker AS last2, g.tracker AS last4, g.tracker AS last8, g.tracker AS view_all,'.$_CAMILA['db']->Concat('g.lat', "','", 'g.lon').' as Coordinate, g.tst, g.tst AS gmaps FROM geotracker_geotracking g JOIN ( SELECT tracker, MAX(tst) AS max_tst FROM geotracker_geotracking GROUP BY tracker ) latest ON g.tracker = latest.tracker AND g.tst = latest.max_tst LEFT OUTER JOIN ${TRACKER} ON ${tracker.tracker}=g.tracker WHERE g.type=\'location\'';
